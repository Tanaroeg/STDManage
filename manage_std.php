	...
	<div class="container">
		...
		<h2>ข้อมูลนักเรียน</h2>
				...
				<?while($objResult = mysqli_fetch_array($objQuery)) { ?>
					<tr class="showrow">
						<td class="showrow"><img width="120px" src="images/<?$Img = $objResult["img"];echo "$Img"?>" /></td>
						...
						<td class="showrow"><a href="edit_std.php?ID=<?$ID = $objResult["ID"];echo "$ID"?>" class="yellow-btn">แก้ไขข้อมูล</a></td>
						<td class="showrow"><a href="delete_std.php?ID=<?$ID = $objResult["ID"];echo "$ID"?>" class="red-btn" onClick="Javascript:return confirm('ยืนยันการลบข้อมูล')">ลบข้อมูล</a></td>
					</tr>
				<?}?>
				</table>
		</div>
